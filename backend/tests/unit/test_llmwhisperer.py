...
async def test_extract_text_success():
    ...
    # Mock submission
    respx.post(f"{base_url}/whisper").mock(
        return_value=httpx.Response(202, json={"whisper_hash": "test-hash"})
    )
    ...
    # Mock retrieval
    respx.get(
        f"{base_url}/whisper-retrieve", params={"whisper_hash": "test-hash"}
    ).mock(return_value=httpx.Response(200, json={"result_text": "Extracted content"}))
...
async def test_extract_text_status_error():
    ...
    base_url = "https://llmwhisperer-api.eu-west.unstract.com/api/v2"
    respx.post(f"{base_url}/whisper").mock(
        return_value=httpx.Response(202, json={"whisper_hash": "h"})
    )
    respx.get(f"{base_url}/whisper-status").mock(
...
async def test_extract_text_unknown_status():
    ...
    respx.get(f"{base_url}/whisper-retrieve").mock(
        return_value=httpx.Response(200, json={"result_text": "ok"})
    )
