...
async def test_upload_cv_success(mock_user):
    ...
    with patch("app.cv.router.whisperer_extract", new_callable=AsyncMock, return_value="Extracted Text"), \
         patch("app.cv.router.classify_entries", new_callable=AsyncMock, return_value=mock_result), \
...
async def test_upload_cv_empty_extraction(mock_user):
    with patch("app.cv.router.whisperer_extract", new_callable=AsyncMock, return_value="  "):
...
async def test_upload_cv_no_nodes(mock_user):
    ...
    with patch("app.cv.router.whisperer_extract", new_callable=AsyncMock, return_value="Some text"), \
         patch("app.cv.router.classify_entries", new_callable=AsyncMock, return_value=mock_result):
        files = {"file": ("test.pdf", b"pdf", "application/pdf")}
        response = client.post("/cv/upload", files=files)
        assert response.status_code == 400
...
async def test_upload_cv_timeout(mock_user):
    with patch("app.cv.router.whisperer_extract", new_callable=AsyncMock, side_effect=TimeoutError("Timeout")):
        files = {"file": ("test.pdf", b"pdf", "application/pdf")}
        response = client.post("/cv/upload", files=files)
...
async def test_confirm_cv_with_merge_and_rels(mock_user, mock_neo4j):
    ...
    confirm_data = {
        "nodes": [
            {"node_type": "work_experience", "properties": {"company": "Google", "title": "SWE"}},
...
async def test_upload_cv_generic_exception(mock_user):
    with patch("app.cv.router.whisperer_extract", new_callable=AsyncMock, side_effect=Exception("Boom")):
        files = {"file": ("test.pdf", b"pdf", "application/pdf")}
        response = client.post("/cv/upload", files=files)
