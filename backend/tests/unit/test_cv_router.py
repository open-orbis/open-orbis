...
@patch("app.cv.router.whisperer_extract")
@patch("app.cv.router.classify_entries")
@patch("app.cv.router.counter")
def test_upload_cv_success(mock_counter, mock_classify, mock_whisperer, client):
    mock_whisperer.return_value = "Extracted text"
...
@patch("app.cv.router.whisperer_extract")
def test_upload_cv_extraction_failed(mock_whisperer, client):
    mock_whisperer.return_value = "  "  # Empty extraction
...
@patch("app.cv.router.whisperer_extract")
def test_upload_cv_timeout(mock_whisperer, client):
    mock_whisperer.side_effect = TimeoutError("Timeout")
    file = BytesIO(b"%PDF-1.4 test")
    response = client.post(
        "/cv/upload", files={"file": ("test.pdf", file, "application/pdf")}
    )
