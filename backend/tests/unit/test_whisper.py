...
async def test_transcribe_audio_success():
    ...
    respx.post(f"{base_url}/asr").mock(return_value=httpx.Response(
...
async def test_transcribe_audio_different_formats():
    ...
    assert "audio/wav" in respx.calls.last.request.content.decode("utf-8", errors="ignore")
    
    await transcribe_audio(b"d", "test.ogg")
    assert "audio/ogg" in respx.calls.last.request.content.decode("utf-8", errors="ignore")
...
async def test_transcribe_audio_error():
    ...
    base_url = "http://localhost:9000"
    respx.post(f"{base_url}/asr").mock(return_value=httpx.Response(500, text="Internal Server Error"))
