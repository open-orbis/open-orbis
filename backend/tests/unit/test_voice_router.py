...
async def test_voice_transcribe_success(mock_user):
    with patch("app.cv.voice_router.transcribe_audio", new_callable=AsyncMock, return_value="Transcribed text"):
...
async def test_voice_transcribe_empty(mock_user):
    with patch("app.cv.voice_router.transcribe_audio", new_callable=AsyncMock, return_value=""):
        files = {"file": ("audio.webm", b"fake audio content" * 10, "audio/webm")}
        response = client.post("/cv/voice-transcribe", files=files)
        assert response.status_code == 200
...
async def test_voice_transcribe_failure(mock_user):
    with patch("app.cv.voice_router.transcribe_audio", new_callable=AsyncMock, side_effect=Exception("Failed")):
        files = {"file": ("audio.webm", b"fake audio content" * 10, "audio/webm")}
        response = client.post("/cv/voice-transcribe", files=files)
...
async def test_voice_classify_success(mock_user):
    ...
    with patch("app.cv.voice_router.classify_entries", new_callable=AsyncMock, return_value=mock_result), \
...
async def test_voice_classify_failure(mock_user):
    with patch("app.cv.voice_router.classify_entries", new_callable=AsyncMock, side_effect=Exception("Failed")):
        response = client.post("/cv/voice-classify", json={"text": "I know Java"})
        assert response.status_code == 500
