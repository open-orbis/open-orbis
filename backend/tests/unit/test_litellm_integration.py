...
async def test_call_llm_error_handling(mock_settings):
    ...
    with patch("litellm.acompletion", side_effect=Exception("LiteLLM Error")):
...
async def test_classify_entries_claude_cli_fallback(mock_settings):
    ...
    with patch("app.cv.claude_classifier.call_claude", new_callable=AsyncMock, return_value=GOOD_LLM_RESPONSE) as mock_cli:
...
async def test_classify_entries_claude_api_call(mock_settings):
    ...
    with patch("app.cv.ollama_classifier._call_llm", new_callable=AsyncMock, return_value=GOOD_LLM_RESPONSE) as mock_llm:
        result = await classify_entries("Some CV text")
