...
import { BETA_CODE_STORAGE_KEY, InviteError, useAuthStore } from '../stores/authStore';
import { LINKEDIN_INVITE_ERROR_KEY } from './LinkedInCallbackPage';
...
function SignInButtons({
  ...
  onGoogleLogin: () => void;
  signingInProvider: 'google' | 'linkedin' | null;
  disabled?: boolean;
  accessCode: string;
    ...
    <div className="flex flex-col gap-3 w-full max-w-xs">
      <div className="flex flex-col gap-1.5">
        ...
        <input
          ...
          placeholder="es. orbis-launch"
          autoComplete="off"
          spellCheck={false}
          className="bg-white/[0.04] border border-white/[0.08] focus:border-purple-500/40 text-white text-sm rounded-xl px-4 py-2.5 outline-none transition-colors placeholder:text-white/20"
        />
      </div>
      <button
      ...
      {inviteError && (
        <div className="text-amber-300/90 text-xs leading-relaxed bg-amber-500/5 border border-amber-500/15 rounded-lg px-3 py-2.5">
          {inviteError}
        </div>
      )}
    </div>
...
export default function LandingPage() {
  ...
  const [accessCode, setAccessCode] = useState<string>(
    () => sessionStorage.getItem(BETA_CODE_STORAGE_KEY) ?? '',
  );
  const [inviteError, setInviteError] = useState<string | null>(null);

  // Pre-fill from ?code= deep link (e.g. orbis.app/?code=newsletter-2026).
  // We persist into sessionStorage so the value survives the LinkedIn OAuth
  // round-trip.
  useEffect(() => {
    const fromUrl = searchParams.get('code');
    if (fromUrl) {
      setAccessCode(fromUrl);
      sessionStorage.setItem(BETA_CODE_STORAGE_KEY, fromUrl);
    }
  }, [searchParams]);

  // Pick up an invite error stashed by the LinkedIn callback during a failed
  // round-trip and surface it on the landing page.
  useEffect(() => {
    const stashed = sessionStorage.getItem(LINKEDIN_INVITE_ERROR_KEY);
    if (stashed && stashed in INVITE_ERROR_MESSAGES) {
      setInviteError(INVITE_ERROR_MESSAGES[stashed as InviteError['code']]);
      sessionStorage.removeItem(LINKEDIN_INVITE_ERROR_KEY);
    }
  }, []);
  ...
  const handleAccessCodeChange = (value: string) => {
    ...
    setInviteError(null);
    if (value.trim()) {
      sessionStorage.setItem(BETA_CODE_STORAGE_KEY, value.trim());
    } else {
  ...
  const handleLoginError = (e: unknown) => {
    ...
    if (e instanceof InviteError) {
      setInviteError(INVITE_ERROR_MESSAGES[e.code] ?? INVITE_ERROR_MESSAGES.unknown);
    } else {
      setInviteError(INVITE_ERROR_MESSAGES.unknown);
    }
  };
