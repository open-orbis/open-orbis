...
import PrintCard from '../components/cv/PrintCard';

// ── Modals ──

function SharePanel({ orbId, person, onClose }: { orbId: string; person: any; onClose: () => void }) {
  ...
  const handlePrint = async () => {
    setPrinting(true);
    trackEvent('print_card_click', 'SharePanel');
    try {
      // Small delay to ensure the hidden component is rendered
      await new Promise(r => setTimeout(r, 100));
      const element = document.getElementById('print-card-template');
      if (!element) throw new Error('Template not found');

      const canvas = await html2canvas(element, { scale: 3, useCORS: true });
      const imgData = canvas.toDataURL('image/png');
      
      // Standard business card: 85x55mm
      const pdf = new jsPDF('l', 'mm', [85, 55]);
      pdf.addImage(imgData, 'PNG', 0, 0, 85, 55);
      pdf.save(`${orbId}-business-card.pdf`);
    } catch (e) {
      ...
      >
        ...
        <div className="flex gap-3">
          <button 
            onClick={handlePrint} 
            disabled={printing}
            className="flex-1 bg-white hover:bg-gray-100 text-black font-semibold py-2 rounded-lg transition-colors text-sm flex items-center justify-center gap-2 shadow-lg"
          >
            {printing ? (
            ...
            {printing ? 'Generating...' : 'Print Physical Card'}
          </button>
          <button onClick={onClose} className="flex-1 border border-gray-600 text-gray-300 hover:bg-gray-800 font-medium py-2 rounded-lg transition-colors text-sm">Close</button>
        </div>
        ...
        <div className="fixed -left-[10000px] -top-[10000px]">
          <PrintCard 
            ...
            headline={person.headline || 'Professional Knowledge Graph'} 
            shareUrl={shareUrl} 
          />
        </div>
      </motion.div>
...
export default function OrbViewPage() {
  ...
  return (
    <div className="min-h-screen bg-black relative">
      ...
      {/* ── Animated Panels ── */}
      <AnimatePresence>
        {showShare && <SharePanel key="share" orbId={orbId} person={data.person} onClose={() => setShowShare(false)} />}
      </AnimatePresence>
      <AnimatePresence>
