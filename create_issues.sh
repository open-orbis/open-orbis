...
# Creates 19 GitHub issues for the Orbis TODO list

REPO="Brotherhood94/orb_project"

echo "Creating 19 issues for $REPO..."
echo ""

# --- A. Core Graph & Data Layer ---

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[A1] Test GraphRAG end-to-end" \
  --body "$(cat <<'EOF'
## Category: Core Graph & Data Layer
...
**Priority:** High
EOF
)" && echo "Created A1" || echo "FAILED A1"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[A2] Fix graph visualization consistency" \
  --body "$(cat <<'EOF'
## Category: Core Graph & Data Layer
...
**Priority:** High
EOF
)" && echo "Created A2" || echo "FAILED A2"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[A3] Graph-level similarity for HR queries" \
  --body "$(cat <<'EOF'
## Category: Core Graph & Data Layer

Individual nodes have vector embeddings, but there's no way to compute similarity between an HR query and the *entire* graph. Research approaches:
...
**Priority:** High
EOF
)" && echo "Created A3" || echo "FAILED A3"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[A4] Implement fine-grained access control" \
  --body "$(cat <<'EOF'
## Category: Core Graph & Data Layer
...
**Priority:** High
EOF
)" && echo "Created A4" || echo "FAILED A4"

# --- B. Onboarding & CV Processing ---

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[B1] Test voice & LinkedIn onboarding with Claude models" \
  --body "$(cat <<'EOF'
## Category: Onboarding & CV Processing
...
**Priority:** High
EOF
)" && echo "Created B1" || echo "FAILED B1"

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[B2] Email scraping → friend invite with rewards" \
  --body "$(cat <<'EOF'
## Category: Onboarding & CV Processing
...
**Priority:** Medium
EOF
)" && echo "Created B2" || echo "FAILED B2"

# --- C. User Experience & Interface ---

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[C1] User click telemetry" \
  --body "$(cat <<'EOF'
...
**Priority:** Medium
EOF
)" && echo "Created C1" || echo "FAILED C1"

gh issue create --repo "$REPO" --label "low-priority" \
  --title "[C2] A2UI (AI-to-UI) testing" \
  --body "$(cat <<'EOF'
## Category: User Experience & Interface
...
**Priority:** Low
EOF
)" && echo "Created C2" || echo "FAILED C2"

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[C3] Interface walkthrough / guided tour" \
  --body "$(cat <<'EOF'
## Category: User Experience & Interface
...
**Priority:** Medium
EOF
)" && echo "Created C3" || echo "FAILED C3"

gh issue create --repo "$REPO" --label "low-priority" \
  --title "[C4] Print orb + QR code" \
  --body "$(cat <<'EOF'
## Category: User Experience & Interface

Generate a printable version of the orb (static graph snapshot or styled summary) with a QR code that links to the live 3D interactive graph. Use case: hand out at conferences, attach to physical resumes. Implement as an export option (PDF with embedded QR). The QR links to the public orb URL (orbis.io/{orb_id}).

**Priority:** Low
EOF
)" && echo "Created C4" || echo "FAILED C4"

# --- D. Platform & Ecosystem ---

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[D1] User playground for orb usage" \
  --body "$(cat <<'EOF'
## Category: Platform & Ecosystem

Give users a sandbox where they can see how their orb works in practice: paste their orb link, see what an LLM extracts, preview MCP tool responses, test different queries. Consider making it pluggable with Lovable (user pastes orb link → Lovable generates a portfolio site). This demonstrates the value proposition directly.

**Priority:** Medium
EOF
)" && echo "Created D1" || echo "FAILED D1"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[D2] Cross-platform MCP client" \
  --body "$(cat <<'EOF'
## Category: Platform & Ecosystem
...
The client connects to Orbis's MCP endpoint and exposes the 5 orb query tools.

**Priority:** High
EOF
)" && echo "Created D2" || echo "FAILED D2"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[D3] MCP query monetization" \
  --body "$(cat <<'EOF'
## Category: Platform & Ecosystem

Design a billing model for MCP queries. Options:
1. Per-query pricing ($0.01-0.05/query)
...
**Priority:** High
EOF
)" && echo "Created D3" || echo "FAILED D3"

gh issue create --repo "$REPO" --label "low-priority" \
  --title "[D4] Orb as identity/login provider" \
  --body "$(cat <<'EOF'
## Category: Platform & Ecosystem
...
**Priority:** Low
EOF
)" && echo "Created D4" || echo "FAILED D4"

# --- E. Social Graph & Network ---

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[E1] Hidden social graph (connection system)" \
  --body "$(cat <<'EOF'
## Category: Social Graph & Network
...
**Priority:** Medium
EOF
)" && echo "Created E1" || echo "FAILED E1"

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[E2] Competitive advantage: instant precision search" \
  --body "$(cat <<'EOF'
## Category: Social Graph & Network
...
**Priority:** High
EOF
)" && echo "Created E2" || echo "FAILED E2"

# --- F. Testing & Quality ---

gh issue create --repo "$REPO" --label "high-priority" \
  --title "[F1] Platform-wide testing strategy" \
  --body "$(cat <<'EOF'
...
**Priority:** High
EOF
)" && echo "Created F1" || echo "FAILED F1"

gh issue create --repo "$REPO" --label "medium-priority" \
  --title "[F2] Agent-based UI testing (Claude + browser)" \
  --body "$(cat <<'EOF'
## Category: Testing & Quality
...
**Priority:** Medium
EOF
)" && echo "Created F2" || echo "FAILED F2"

# --- G. Mobile & Extended Platforms ---

gh issue create --repo "$REPO" --label "low-priority" \
  --title "[G1] Mobile app feasibility (Android/iOS)" \
  --body "$(cat <<'EOF'
## Category: Mobile & Extended Platforms
...
**Priority:** Low
EOF
)" && echo "Created G1" || echo "FAILED G1"

gh issue create --repo "$REPO" --label "low-priority" \
  --title "[G2] Email integration for behavior insights" \
  --body "$(cat <<'EOF'
## Category: Mobile & Extended Platforms
...
**Priority:** Low
EOF
)" && echo "Created G2" || echo "FAILED G2"
